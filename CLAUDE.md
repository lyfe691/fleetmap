...
scripts/prove-driver-session.ts      human-gated prod proof: exchange + refresh + authenticated GPS write (deployment.md §9)
scripts/gen-selfhost-keys.ts         JWT secret + anon/service keys for a self-hosted Supabase stack
docs/driver-session-api.md            the exchange contract for Roman's app
docs/HANDOFF.md                       current state, people, invariants, working notes (read second)
docs/archive/                         superseded history: the 2026-07/08 handoff log, the executed advisor plans
...
- [x] **M17 — Supabase local + self-host:** dev runs the local CLI stack (`supabase/config.toml`, 4432x ports, explicit grants migration 0015); prod migrated off managed cloud onto the vendored `supabase-docker/` stack on the VPS (Kong behind `sb.fleet.ysz.life`, data + auth users moved with logins intact, nightly pg_dump backups). App images build locally and ship as tars. The managed-cloud project was retired the same day (no rollback; durability = nightly pg_dump on the box). Spec: `docs/specs/2026-07-20-supabase-local-and-selfhost-design.md`.
...
- [x] **M20 — driver auth federation:** the rider app posts a 2-minute Bubble Box `fleetAuthToken` to `POST /api/driver-session`; the internal `driver-session` service verifies it via Bubble Box's `POST /api/v2/fleet/verify-rider-token` (authenticated with the same fleet token the sync uses), maps the returned rider `id` to `vehicles.rider_ref`, auto-provisions the passwordless Supabase driver on first login, and returns a Supabase `access_token` + `refresh_token`. The request-lifecycle logging in `driver-session` has been live since 2026-08-10; the human-gated §9 production proof (`docs/deployment.md`) passed on 2026-08-13 against the staging credentials; and Roman's device build completed the flow in production on 2026-08-24 (GPS live on the dashboard). Staging test riders 6 (Zurich) and 13 (Basel) are the mapped vehicles today. Contract: `docs/driver-session-api.md`; spec: `docs/specs/2026-07-31-driver-session-cutover-design.md`.
- **Next (decided 2026-09-07):** production is a fresh install on the company's instance (Severin provides it; `docs/deployment.md` §0–§9, own hostnames + secrets, hostname-parametrized via `FLEET_HOST`/`SUPABASE_HOST`), while the VPS stays the staging environment. Go-live there: Bubble Box production credentials (`https://bubblebox.ch`; Dmytro is preparing the production deploy and will create the fleet user), production rider-id → vehicle mapping, Roman's production build with the three constants, §9 proof, uptime monitor, offsite backups (checklist at the end of `docs/deployment.md`). Telematics remains dropped, not deferred.
