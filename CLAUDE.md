...
- **Next (decided 2026-09-07):** move the stack from the VPS to the company server under company hostnames (`docs/deployment.md` §11 — deployment is hostname-parametrized via `FLEET_HOST`/`SUPABASE_HOST`), then orders go-live there: production Bubble Box credentials (blocked on Dmytro's answer about the production fleet API), production rider-id → vehicle mapping (staging ids 6/13 must not carry over), Roman's release with the final hostnames, secret rotation, uptime monitor, offsite backups (checklist at the end of `docs/deployment.md`). Telematics remains dropped, not deferred.
