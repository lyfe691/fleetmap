...
- [x] **M20 — driver auth federation (current truth, 2026-07-31):** code is complete locally. The public request remains `{ token: fleetAuthToken }`; `lib/driver-auth/verify.ts` privately calls Bubble Box's `POST /api/v2/fleet/verify-rider-token` with Fleetmap's cached fleet-service `loginToken` in `accessToken` and the submitted value as `{ riderAuthToken }`, then reads the top-level rider `id`. The exchange maps `rider_ref`, uses or auto-provisions the passwordless Supabase driver, and returns Supabase `access_token` + `refresh_token`. Roman must exchange the approximately two-minute `fleetAuthToken` immediately, persist the Supabase session, and reacquire a fresh fleet token through `GET /api/v2/riders/fleet-auth-token` only after terminal Supabase refresh failure while the rider `loginToken` remains valid. Production has the 2026-07-29 CORS/liveness image, but **does not yet have the verification-swap artifact**. The remaining real-token proof is blocked only by Roman's supplied fixture: its legacy login path returned `404`, and the documented current login returned `401` on 2026-07-31. This does not block local code or release-artifact readiness. No database migration is required. Contract: `docs/driver-session-api.md`; deployment: `docs/deployment.md`.
- **Next:** build and ship all three explicit `:latest` images from the dev machine, create the four-variable `.env.driver-session` first, and run the safe-stdin proof with a controlled rider mapping before Roman releases. Orders remain independent: put production Bubble Box credentials in `/opt/fleetmap/.env`, map each production rider id from that environment to one vehicle, then watch `/api/health` (`driver_session` included). Telematics remains dropped, not deferred.
