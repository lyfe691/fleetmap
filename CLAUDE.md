...
- [x] **M20 — driver auth federation:** the double-login wart killed per spec `2026-07-13` (+ its 2026-07-22 review): `workers/driver-session.ts` exchanges a Bubble Box rider JWT for a Supabase session — verify RS256 (`lib/driver-auth/verify.ts`; rider object = trust boundary, fleet/staff tokens rejected) → map `rider_ref` → use or **auto-provision** the driver user (no password, RLS principal only) → mint via admin magiclink + verifyOtp. Internal compose service behind Caddy `/api/driver-session`; dev stand-in keypair via `pnpm driver-test-token`. **Deployed and proven in prod 2026-07-27**: a stand-in-signed token against `https://fleet.ysz.life/api/driver-session` returned a session, auto-provisioned the driver, claimed the van, and wrote GPS through RLS (test data removed). Contract for Roman: `docs/driver-session-api.md`. **Verification is being replaced (2026-07-27, agreed in the three-way chat):** Bubble Box issues Roman a fleet-scoped token and exposes a verification endpoint we call, instead of us verifying a real rider JWT against their public key — better blast radius, plus revocation. Only `lib/driver-auth/verify.ts` changes; the contract, `rider_ref` lookup, provisioning, mint and RLS all stand. See the spec's 2026-07-27 section. The public key + rider-sample asks are dead.
