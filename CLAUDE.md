...
- Later: prod rollout of the sync (BB env in `/opt/fleetmap/.env`, provision the real fleet — auth user + vehicle + `rider_ref` per driver, apply 0016 + ship fresh images), prove it live; telematics integrate-or-drop decision. Roman re-points his app at `https://sb.fleet.ysz.life` + the new publishable key. ← next
