...
- Next — prod is fully caught up (2026-07-27: all three images shipped, schema 0016, M20 live and its happy path proven against prod with a stand-in key). Two independent tracks remain, both gated on Dmytro: **orders** — BB env in `/opt/fleetmap/.env`, one vehicle row + `rider_ref` per rider in his prod feed, then watch `/api/health`; **login** — he builds `/fleet/verify-token` (confirmed 2026-07-27: same rider id as `/fleet/rider-routes`, same `/fleet/authentication-token` auth — no new credentials, no `rider_ref` rework; only the request/response shape is still open), we swap `lib/driver-auth/verify.ts` to call it, then Roman ships one release per `docs/driver-session-api.md`. Keep them uncoupled: orders do not wait on login. Telematics is settled and dead — dropped, not deferred (see the monitoring-console bullet).
