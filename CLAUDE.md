...
- **Next (2026-09-07, after the server move):** orders go-live on the company box: Bubble Box production credentials (`https://bubblebox.ch`; Dmytro is preparing the production deploy and will create the fleet user), production rider-id → vehicle mapping, §9 proof with a real rider, secret rotation, uptime monitor, offsite backups (checklist at the end of `docs/deployment.md`). Roman needs no change (same hostnames and key). Stop the old VPS stacks once DNS caches expire. Telematics remains dropped, not deferred.
