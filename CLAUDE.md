...
app/api/health/route.ts     health probe — Supabase + OSRM + driver-session + sync freshness (GET /api/health)
...
workers/driver-session.ts             BB fleetAuthToken → Supabase session exchange (internal service, Caddy route /api/driver-session)
lib/bubblebox/translate.ts            pure rider-route → orders/stops translation (unit-tested)
lib/bubblebox/client.ts               BB fleet API client — token mint/cache/single 401 re-mint (unit-tested)
lib/driver-auth/verify.ts             BB token verification — private verify-rider-token call + rider trust boundary
lib/driver-auth/exchange.ts           token → session decision logic, deps injected (unit-tested)
scripts/verify-live-token.ts          run a real 2-minute BB rider token through verify + exchange
scripts/mint-fleet-auth-token.ts      self-serve a fresh staging fleetAuthToken (test rider) — pipes into verify-live-token
...
redeploy.sh                 VPS: git pull + load the local three-image archive + restart with --no-build
...
lib/console/assumed.ts      the static "Depot" route-start label (vehicles have no origin column)
...
- **The dashboard is the monitoring console.** `app/dashboard` → display-code gate → `ConsoleShell` (`components/console/*`): a 3-region touchscreen layout (sidebar nav + fleet rail + tracking/map/history) on shadcn + next-themes light/dark. `components/map/fleet-map-view.tsx` is the reused, theme-aware map surface (`lib/map-theme.ts`); stops render in a two-tier "focus mode" keyed on `selectedId` — fleet view shows small on-line waypoint dots, selecting a van upgrades its stops to seq-numbered badges (projected-ETA pill on the next stop) and dims the other vans' lines/dots to ~15% (spec: `docs/specs/2026-07-14-stops-ui-focus-mode.md`). **Every panel shows real data.** The fabricated telematics/cargo/manifest panels were deleted on 2026-07-13, not faked: van load is derived from completed pickups minus deliveries, and "distance today" is a server-side aggregate (`vehicle_distance_m`, 0010). All that survives in `lib/console/assumed.ts` is the static `"Depot"` route-start label, because `vehicles` has no origin column. Real weight/temperature/fuel would need in-vehicle hardware the fleet does not have. The History tab is real: route replay from `vehicle_positions` (0008 read path, `lib/replay.ts` math, playback UI in `history-view.tsx`).
...
pnpm verify-live-token <token> [url] # test a real BB rider token before its 2 min expires
pnpm --silent mint-fleet-auth-token  # self-serve a fresh 2-min fleetAuthToken (staging test rider); pipe into verify-live-token
...
**Demo against prod with fake-gps:** `fake-gps` is a local tool — it needs `SUPABASE_SECRET_KEY` (admin setup + reading stops). Since M17 prod is the self-hosted stack (`https://sb.fleet.ysz.life`) and its secret key lives only on the VPS, so a prod demo means deliberately copying that key to the dev machine for one run and removing it after (the old `.env.cloud` recipe targeted the retired managed-cloud project). Stops need `seed-stops` against prod too (prod's `DISPATCHER_INGEST_SECRET`), and local OSRM must be up for route geometry (prod OSRM is internal-only). Two things changed since this recipe was written: the M19 geofence retirement means nothing completes stops during a demo (movement, route greying, ETAs and lateness still animate; arrival deltas and done-states stay static), and fake vans carry no `rider_ref`, so the live sync ignores them — they render beside real synced vans rather than fighting over a marker:

```
FAKE_GPS_API_URL=https://fleet.ysz.life/api/location NEXT_PUBLIC_SUPABASE_URL=https://sb.fleet.ysz.life NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=... SUPABASE_SECRET_KEY=... pnpm fake-gps
```

It writes into prod's Supabase — use one throwaway city if possible and delete the fake vans/users afterwards.
...
- [x] **M20 — driver auth federation (current truth, 2026-08-10):** The public request remains `{ token: fleetAuthToken }`; `lib/driver-auth/verify.ts` privately calls Bubble Box's `POST /api/v2/fleet/verify-rider-token` with Fleetmap's cached fleet-service `loginToken` in `accessToken` and the submitted value as `{ riderAuthToken }`, then reads the top-level rider `id`. The exchange maps `rider_ref`, uses or auto-provisions the passwordless Supabase driver, and returns Supabase `access_token` + `refresh_token`. Roman must exchange the approximately two-minute `fleetAuthToken` immediately, persist the Supabase session, and reacquire a fresh fleet token through `GET /api/v2/riders/fleet-auth-token` only after terminal Supabase refresh failure while the rider `loginToken` remains valid. The verification cutover in commit `530b117` was deployed as all three images and production services were healthy on 2026-08-10, including `/api/health` coverage for `driver_session`. **2026-08-11:** the Bubble Box side is fully proven from outside — a self-served staging token ran rider login → `fleet-auth-token` → `verify-rider-token` → `200 {id}` with our fleet account (rights were never the issue; every historical 403 was an invalid token). Rider login is `POST {BB_API_URL}/shop/api/v1/en/security/check-login` with JSON `{username, password}` — the `/shop` prefix was the missing piece — and `pnpm mint-fleet-auth-token` self-serves fresh 2-minute tokens (staging test-rider creds in `.env`). The deployed cutover image was confirmed externally by timing (token rejections include the Bubble Box round trip). Still unproven: the client flow — no TestFlight build with the new exchange exists (the 27.07 build predates the `fleetAuthToken` feature; Roman had the pre-cutover contract doc until the corrected one was sent 2026-08-11). **2026-08-13:** the request-lifecycle diagnostic image turns out to have been live in prod since 2026-08-10 10:47 UTC — the worker log shows `request_received`/`request_completed` events plus the §6 verification sequence right at startup; the rollout was run and verified on 10.08 but never recorded. Same day, Roman exercised the new flow from a web dev harness (`localhost:5173` origin): his exchanges passed Bubble Box verification and failed only as `unmapped_rider` (rider 6, one attempt as rider 13), so the public contract and his client flow are correct. Later that day the §9 controlled production proof ran and **passed** (typed-approval preflight → real-token exchange → forced refresh → authenticated GPS write → guarded cleanup, sync resumed), standing test vehicles were mapped for staging riders 6 (`BB test — Rider Zurich City 1`) and 13 (`BB test — Rider Basel`), and at 12:57 UTC **Roman's own client completed a real exchange** (`driver_autoprovisioned` + `session_minted`, rider 6; `assigned_user_id` set) — the client flow is proven, from his dev harness. Order side, verified same day: the sync matches both riders; the 13.08 staging routes carry zero orders (skeleton depot points), while 14.08 has real orders (Basel 4 stops, Zurich 1) that attach automatically. No database migration is required. Contract: `docs/driver-session-api.md`; deployment: `docs/deployment.md`.
- **Next:** one controlled TestFlight retry once Roman ships a build with the new exchange flow (the 2026-08-13 success ran from his web dev harness, not a device build). Orders remain independent: the sync is live in prod against the staging fleet API (fresh heartbeat in `/api/health`); go-live means production Bubble Box credentials in `/opt/fleetmap/.env` and mapping each production rider id to one vehicle. Telematics remains dropped, not deferred.
