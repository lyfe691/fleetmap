...
- Later: prod rollout of the sync (BB env in `/opt/fleetmap/.env`, provision vans — vehicle row + `rider_ref` each; driver users auto-provision via M20), apply 0016 + ship fresh images, prove it live; telematics integrate-or-drop decision. Roman: one release per `docs/driver-session-api.md` (re-point + exchange call, drops the second login). ← next
