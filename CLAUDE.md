...
- [x] **M19 — retirements:** `/dispatch` + geofence deleted, `stops.address` dropped (0016) — see the retired-surfaces convention bullet. Built on branch `retire-dispatch-geofence`; merges after the sync's first proven prod day.
- Later: prod rollout of the sync (BB env in `/opt/fleetmap/.env`, provision the real fleet — auth user + vehicle + `rider_ref` per driver), prove it live, merge M19; telematics integrate-or-drop decision. Roman re-points his app at `https://sb.fleet.ysz.life` + the new publishable key. ← next
