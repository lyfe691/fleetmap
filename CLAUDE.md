...
- [x] **M20 — driver auth federation:** the double-login wart killed per spec `2026-07-13` (+ its 2026-07-22 review): `workers/driver-session.ts` exchanges a Bubble Box rider JWT for a Supabase session — verify RS256 (`lib/driver-auth/verify.ts`; rider object = trust boundary, fleet/staff tokens rejected) → map `rider_ref` → use or **auto-provision** the driver user (no password, RLS principal only) → mint via admin magiclink + verifyOtp. Internal compose service behind Caddy `/api/driver-session`; a dev stand-in keypair stood in for their key (since retired). **Deployed and proven in prod 2026-07-27**: a stand-in-signed token against `https://fleet.ysz.life/api/driver-session` returned a session, auto-provisioned the driver, claimed the van, and wrote GPS through RLS (test data removed). Contract for Roman: `docs/driver-session-api.md`. **Verification was replaced on 2026-07-29:** `lib/driver-auth/verify.ts` no longer checks an RS256 signature. It POSTs the rider token to Bubble Box's `POST /api/v2/fleet/verify-rider-token` through `lib/bubblebox/client.ts` (same fleet credential as the sync) and reads the top-level `id` from their `{ id, fullName }` response — the same rider id `rider_ref` already holds. Status split is the contract: **403 = bad or expired rider token → 401 to the driver; anything else (401 on our own credential, 5xx, network) → 500**, never "invalid token". Riders get a purpose-issued `fleetAuthToken` that lives **2 minutes**, so the app must exchange immediately at login and then live off Supabase refresh. `BB_DRIVER_JWT_PUBLIC_KEY_B64` and the stand-in keypair script are retired; `.env.driver-session` now needs `BB_API_URL`/`BB_API_USERNAME`/`BB_API_PASSWORD`. **Not yet proven against a real token** — only the rider app can mint one, use `pnpm verify-live-token` to beat the 2-minute clock.
- Next — prod is fully caught up (2026-07-27: all three images shipped, schema 0016, M20 live and its happy path proven against prod with a stand-in key). Two independent tracks remain, both gated on Dmytro: **orders** — BB env in `/opt/fleetmap/.env`, one vehicle row + `rider_ref` per rider in his prod feed, then watch `/api/health`; **login** — he shipped it on 2026-07-29 as `POST /api/v2/fleet/verify-rider-token` (header `accessToken`, body `{ riderAuthToken }`; the name `/fleet/verify-token` 404s), and riders now get a purpose-issued `fleetAuthToken` that lives **2 minutes**. Blocked: our fleet account gets `403 Zugriff verweigert.` on it while the same token still returns 200 on `/fleet/rider-routes`, and the success shape is undocumented — proven not to be a payload problem (`docs/HANDOFF.md`, 2026-07-29). When it opens: swap `lib/driver-auth/verify.ts` to call it through `lib/bubblebox/client.ts`, move `.env.driver-session` from the stand-in public key to BB creds, ship a driver-session image, re-prove against prod, then Roman ships one release per `docs/driver-session-api.md`. Until then prod verifies against the stand-in key and 401s every real token. Keep them uncoupled: orders do not wait on login. Telematics is settled and dead — dropped, not deferred (see the monitoring-console bullet).
